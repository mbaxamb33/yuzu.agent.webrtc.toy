...
echo "Open the room URL and talk over the bot to barge-in." >&2
...
while [ $(date +%s) -lt $deadline ]; do
  ...
  grep -q '"barge_in_latency_ms"' <<< "$events" && seen_latency=1 || true
...
echo "Timeout waiting for vad_start/stop_tts_sent/tts_stopped(interrupted)/barge_in_latency_ms" >&2
exit 1
